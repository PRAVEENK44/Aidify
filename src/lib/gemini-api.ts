...
export const parseFirstAidInstructions = (generatedText: string) => {
  ...
  const hasEmergency = /(?:emergency|immediate medical attention|call emergency services|seek medical|hospital|urgent|seek professional|paramedics)/i.test(cleanedText);
  let warning = "";
  ...
  if (hasEmergency) {
    const warningPatterns = [
      ...
      /(?:emergency|immediate medical attention|call emergency services|seek medical|hospital|urgent|seek professional|paramedics)([^.]+(?:\.[^.]+)*)/i
    ];
